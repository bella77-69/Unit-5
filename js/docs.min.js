...
jQuery(function () {
  ...
  jQuery(document).ready(function (a) {
    a("body").scrollspy({ target: ".bs-sidebar", offset: 0 }),
    ...
    b.initialize(),
      a("#tokenfield-typeahead").tokenfield({
        typeahead: { source: b.ttAdapter() },
      }),
      a("#tokenfield-2")
        .on("tokenfield:preparetoken", function (a) {
          var b = a.token.value.split("|");
          (a.token.value = b[1] || b[0]),
            (a.token.label = b[1] ? b[0] + " (" + b[1] + ")" : b[0]);
        })
        .on("tokenfield:createtoken", function (b) {
          var c = /\S+@\S+\.\S+/,
            d = c.test(b.token.value);
        ...
        .on("tokenfield:removetoken", function (b) {
          if (b.token.length > 1) {
            var c = a.map(b.token, function (a) {
              return a.value;
            });
            alert(
              b.token.length +
                " tokens removed! Token values were: " +
                c.join(", ")
            );
          } else alert("Token removed! Token value was: " + b.token.value);
        })
        .on("tokenfield:preventduplicate", function (a) {
          alert("Duplicate detected! Token value is: " + a.token.value);
        })
        .tokenfield();
  });
